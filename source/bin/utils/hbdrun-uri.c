/*
 * hbdrun-uri - Utilities for HVML URI (copied from PurC).
 *
...
size_t hbdrun_uri_assemble(char *uri, const char *host, const char* app,
        const char* runner, const char *group, const char *page)
{
    char *start = uri;

    uri = stpcpy(uri, "hbdrun://");
    uri = stpcpy(uri, host);
    uri[0] = '/';
    uri++;
...
char* hbdrun_uri_assemble_alloc(const char* host, const char* app,
        const char* runner, const char *group, const char *page)
{
    char* uri;
    static const int schema_len = sizeof(HBDRUN_SCHEMA) - 1;
    ...
    hbdrun_uri_assemble(uri, host, app, runner, group, page);
    return uri;
}
...
bool hbdrun_uri_split(const char *uri,
        char *host, char *app, char *runner, char *group, char *page)
{
    static const unsigned int sz_schema = sizeof(HBDRUN_SCHEMA) - 1;
    unsigned int len;

    if (strncasecmp(uri, HBDRUN_SCHEMA, sz_schema))
        return false;
...
bool hbdrun_uri_split_alloc(const char *uri,
        char **host, char **app, char **runner, char **group, char **page)
{
    static const unsigned int sz_schema = sizeof(HBDRUN_SCHEMA) - 1;
...
bool hbdrun_uri_get_query_value(const char *uri, const char *key,
        char *value_buff)
{
...
bool hbdrun_uri_get_query_value_alloc(const char *uri, const char *key,
        char **value_buff)
{
    const char *value = locate_query_value(uri, key);
