...
{
    LOG_DEBUG("remove webView (%p) from session (%p)\n", webView, sess);
...
{
    void *data;
    if (!sorted_array_find(sess->all_handles, PTR2U64(plainWin), &data)) {
        return PCRDR_SC_NOT_FOUND;
    }
