/*
** BrowserPaneContainer.c -- The implementation of BrowserPaneContainer.
**
...
struct _BrowserPaneContainerClass {
    GObjectClass parent;
};

G_DEFINE_TYPE(BrowserPaneContainer, browser_pane_container,
        G_TYPE_OBJECT)

static void browser_pane_container_init(BrowserPaneContainer *window)
{
}

static void browserPlainWindowConstructed(GObject *gObject)
{
    BrowserPaneContainer *window = BROWSER_PANE_CONTAINER(gObject);
    G_OBJECT_CLASS(browser_pane_container_parent_class)->constructed(gObject);
    ...
    g_warning("Creating pan container: (%d, %d; %d x %d)",
            window->geometry.left, window->geometry.top, w, h);
...
{
    BrowserPaneContainer *window = BROWSER_PANE_CONTAINER(object);
...
static void browserPlainWindowDispose(GObject *gObject)
{
    ...
    G_OBJECT_CLASS(browser_pane_container_parent_class)->dispose(gObject);
}
...
static void browser_pane_container_class_init(BrowserPaneContainerClass *klass)
{
...
browser_pane_container_new(BrowserTabbedWindow *window,
        GObject *container, const char *klass, const RECT *geometry)
{
    BrowserPaneContainer *layout_container =
          BROWSER_PANE_CONTAINER(g_object_new(BROWSER_TYPE_PANE_CONTAINER,
...
HWND browser_pane_container_get_hwnd(BrowserPaneContainer *container)
{
    return container->hwnd;
}

void browser_pane_container_add_child(BrowserPaneContainer *container,
        BrowserPane *pane)
{
    container->children = g_slist_append(container->children, pane);
}

GSList *browser_pane_container_get_children(BrowserPaneContainer *container)
{
